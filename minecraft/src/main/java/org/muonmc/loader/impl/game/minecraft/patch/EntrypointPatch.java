...
import org.muonmc.loader.impl.game.minecraft.MinecraftGameProvider;
...
public class EntrypointPatch extends GamePatch {
	...
	private void finishEntrypoint(Environment environment, ListIterator<AbstractInsnNode> it) {
		String sideName = environment == Environment.CLIENT ? "Client" : "Server";
		/* START OF QUILT */
	...
	public void process(MuonLauncher launcher, String namespace, GamePatchContext context) {
		Environment environment = launcher.getEnvironmentType();
		...
		if (gameEntrypoint == null) {
			...
			if (environment == Environment.CLIENT && mainMethod.instructions.size() < 10) {
			...
			if (gameEntrypoint == null) {
				...
				if (newGameInsn == null && environment == Environment.DEDICATED_SERVER) {
				...
				if (environment == Environment.DEDICATED_SERVER && hasStrInMethod(mainClass.name, mainMethod.name, mainMethod.desc, "Safe mode active, only vanilla datapack will be loaded", classSource)) {
		...
		if (!is20w22aServerOrHigher) {
			for (MethodNode gmCandidate : gameClass.methods) {
				...
				if (environment == Environment.CLIENT && !isApplet && gameMethodQuality < 2) {
		...
		if (environment == Environment.DEDICATED_SERVER) {
			...
			if (!is20w22aServerOrHigher) {
				...
				finishEntrypoint(environment, it);
				patched = true;
			} else {
				...
				finishEntrypoint(environment, it); // Inject the hook entrypoint.
		...
		} else if (environment == Environment.CLIENT && isApplet) {
			...
			if (runDirectory == null) {
				...
				it.add(new InsnNode(Opcodes.ACONST_NULL));
				it.add(new MethodInsnNode(Opcodes.INVOKESTATIC, "org/muonmc/loader/impl/game/minecraft/applet/AppletMain", "hookGameDir", "(Ljava/io/File;)Ljava/io/File;", false));
				it.add(new VarInsnNode(Opcodes.ALOAD, 0));
				finishEntrypoint(environment, it);
			} else {
				// Indev and above.
