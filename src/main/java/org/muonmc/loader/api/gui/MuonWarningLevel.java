...
public enum MuonWarningLevel {
	 ...
	 * to fix. */
	FATAL(MuonLoaderGui.iconLevelFatal()),
	...
	ERROR(MuonLoaderGui.iconLevelFatal()),
	...
	WARN(MuonLoaderGui.iconLevelWarn()),
	...
	INFO(MuonLoaderGui.iconLevelInfo()),

	/** A 'lesser' warning that a mod developer might want to know about. Use this when you don't want to inform the
	 * user that something is wrong. */
	CONCERN(MuonLoaderGui.iconLevelConcern()),

	/** The default {@link MuonWarningLevel}, which indicates nothing special about the node - however the user might
	 * still want to know about it, hence why it's still included in the gui. */
	NONE(null),

	/** A special {@link MuonWarningLevel} which indicates that this node will be hidden if the debug option is turned
	 * off. Use this for very-fine details which are only useful during plugin development, or debugging plugins
	 * specifically. */
	DEBUG_ONLY(null);

	private final MuonLoaderIcon icon;

	private MuonWarningLevel(MuonLoaderIcon icon) {
		this.icon = icon;
	}

	public static MuonWarningLevel getHighest(MuonWarningLevel a, MuonWarningLevel b) {
