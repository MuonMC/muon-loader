...
public interface MuonPluginManager {
	...
	/** Returns a task which will load the specified zip file and returns a path to the root of its contents.
	 * <p>
	 ...
	 * of the {@link MuonLoaderPlugin}s {@link MuonPluginContext#lockZip(Path) locks} it, or if a chosen mod is loaded
	 * from it.
	 * <p>
	 ...
	 * 
	 * @see #loadZipNow(Path) */
	MuonPluginTask<Path> loadZip(Path zip);
	 ...
	 *            <ul>
	 *            <li>{@link MuonLoaderPlugin#scanFolder(Path, ModLocation, PluginGuiTreeNode)}</li>
	 *            <li>{@link MuonLoaderPlugin#scanUnknownFile(Path, ModLocation, PluginGuiTreeNode)}</li>
	 *            <li>{@link #getParent(Path)} of the path passed into
	 *            {@link MuonLoaderPlugin#scanZip(Path, ModLocation, PluginGuiTreeNode)}.</li>
	 *            </ul>
	 * @throws IllegalStateException if called outside a {@link ModLoadOption#convertToMod(Path)} call. (Loader must
	 *             intern all paths for consistency’s sake, which doesn't work very well if you call this too late or
	 *             early). */
	 ...
	 *            {@link MuonLoaderPlugin#scanUnknownFile(Path, boolean, PluginGuiTreeNode)} or the {@link #getParent(Path)} of
	 *            a path passed to {@link MuonLoaderPlugin#scanZip(Path, boolean, PluginGuiTreeNode)}. (Paths in
	...
	/** @param mod The path to the mod. This should always be one that was passed to
	 *            {@link MuonLoaderPlugin#scanUnknownFile(Path, ModLocation, PluginGuiTreeNode)}, or the
	 *            {@link #getParent(Path)} of a path passed to
	 *            {@link MuonLoaderPlugin#scanZip(Path, ModLocation, PluginGuiTreeNode)}. Alternatively you can pass in
	 *            a path returned by {@link #getModPaths()}.
	 * @return A map of potential mod load options, keyed by the {@link MuonPluginContext#pluginId()}. */
	@Nullable
