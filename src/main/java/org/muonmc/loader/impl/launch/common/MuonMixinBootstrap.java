...
import org.muonmc.loader.impl.util.mappings.MixinIntermediaryDevRemapper;
...
public final class MuonMixinBootstrap {
	private MuonMixinBootstrap() { }
	...
	static Set<String> getMixinConfigs(MuonLoaderImpl loader, Environment environment) {
		return loader.getAllMods().stream()
				.map(ModContainer::metadata)
				.filter((m) -> m instanceof FabricLoaderModMetadata)
				.flatMap((m) -> ((FabricLoaderModMetadata) m).getMixinConfigs(environment).stream())
				.filter(s -> s != null && !s.isEmpty())
				.collect(Collectors.toSet());
	}

	public static void init(Environment side, MuonLoaderImpl loader) {
		if (initialized) {
			throw new IllegalStateException("MuonMixinBootstrap has already been initialized!");
		}

		if (MuonLauncherBase.getLauncher().isDevelopment()) {
			MappingConfiguration mappingConfiguration = MuonLauncherBase.getLauncher().getMappingConfiguration();
			...
			if (mappings != null) {
				...
				if (namespaces.contains("intermediary") && namespaces.contains(mappingConfiguration.getTargetNamespace())) {
					...
					try {
						...
						Log.info(LogCategory.MIXIN, "Loaded Muon development mappings for mixin remapper!");
					} catch (Exception e) {
						Log.error(LogCategory.MIXIN, "Muon development environment setup error - the game will probably crash soon!");
						e.printStackTrace();
					}
				}
			}
		...
		MixinBootstrap.init();
		getMixinConfigs(loader, side).forEach(MuonMixinBootstrap::addConfiguration);
