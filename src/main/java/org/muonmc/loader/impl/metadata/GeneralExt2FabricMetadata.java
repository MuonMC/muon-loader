...
public class GeneralExt2FabricMetadata implements FabricLoaderModMetadata {
	...
	public GeneralExt2FabricMetadata(ModMetadataExt meta, ModContainerExt container) {
		...
		for (org.muonmc.loader.api.ModDependency dep : meta.depends()) {
			if (dep instanceof org.muonmc.loader.api.ModDependency.Only) {
				Quilt2FabricModDependency q2f = new Quilt2FabricModDependency(true, (org.muonmc.loader.api.ModDependency.Only) dep);
				depsAndBreaks.add(q2f);
				depends.add(q2f);
			}
		}
		for (org.muonmc.loader.api.ModDependency dep : meta.breaks()) {
			if (dep instanceof org.muonmc.loader.api.ModDependency.Only) {
				Quilt2FabricModDependency q2f = new Quilt2FabricModDependency(false, (org.muonmc.loader.api.ModDependency.Only) dep);
				depsAndBreaks.add(q2f);
				breaks.add(q2f);
			}
		}
	...
	private static UnsupportedOperationException internalError() {
		throw new UnsupportedOperationException("Fabric-internal metadata is not exposed for " + MuonConstants.CAPITAL_BRAND + " mods - since only " + MuonConstants.NAME + " itself may use this.");
	}
	...
	static final class Quilt2FabricModDependency implements ModDependency {
		...
		final org.muonmc.loader.api.ModDependency.Only quiltDep;

		public Quilt2FabricModDependency(boolean isDepends, org.muonmc.loader.api.ModDependency.Only quiltDep) {
		...
		public boolean matches(Version version) {
			if (version instanceof org.muonmc.loader.api.Version) {
				return quiltDep.matches((org.muonmc.loader.api.Version) version);
			} else {
				return quiltDep.matches(org.muonmc.loader.api.Version.of(version.getFriendlyString()));
			}
		}
