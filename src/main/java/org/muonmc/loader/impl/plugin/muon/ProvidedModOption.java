...
public class ProvidedModOption extends ModLoadOption implements AliasedLoadOption {
	...
	public MuonLoaderText describe() {
		return MuonLoaderText.translate("solver.option.mod.quilt_impl", "provided", id() + " " + version(), provider.describe());
	}
	...
	public void populateModsTabInfo(PluginGuiTreeNode guiNode) {
		...
		PluginGuiTreeNode c = guiNode.addChild(MuonLoaderText.of(provider.id() + " " + provider.version()));
		c.mainIcon(provider.modTypeIcon());
		c.addChild(MuonLoaderText.of(loader().manager().describePath(provider.from())))
