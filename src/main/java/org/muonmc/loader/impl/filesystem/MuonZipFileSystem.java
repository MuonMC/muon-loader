...
import org.muonmc.loader.impl.util.DisconnectableByteChannel;
...
import org.muonmc.loader.impl.util.MuonLoaderCleanupTasks;
 ...
 * only use this if the backing path supports efficient random access (generally {@link MuonMemoryFileSystem} supports
 * this if it's not read-only, or the "compress" constructor argument is false). */
@MuonLoaderInternal(MuonLoaderInternalType.INTERNAL)
public class MuonZipFileSystem extends MuonMapFileSystem<MuonZipFileSystem, MuonZipPath>
	implements ReadOnlyFileSystem {

	static final boolean DEBUG_TEST_READING = false;

	final WeakReference<MuonZipFileSystem> thisRef = new WeakReference<>(this);
	final ZipSource source;

	public MuonZipFileSystem(String name, Path zipFrom, String zipPathPrefix) throws IOException {
		super(MuonZipFileSystem.class, MuonZipPath.class, name, true);
	...
	private void initializeFromZip(InputStream fileStream, String zipPathPrefix) throws IOException {
		try (CountingInputStream counter = new CountingInputStream(fileStream); //
			...
			while ((entry = zip.getNextEntry()) != null) {
				...
				MuonZipPath path = getPath(entryName);
	...
	private void readDirectory(MuonZipPath path, DataInputStream stream, String zipPathPrefix) throws IOException {
		...
		for (int i = 0; i < childFiles; i++) {
			...
			MuonZipPath filePath = path.resolve(new String(nameBytes, StandardCharsets.UTF_8));
	...
	/** Constructs a new {@link MuonZipFileSystem} that only exposes a single sub-folder of a larger
	 * {@link MuonZipFileSystem}. */
	public MuonZipFileSystem(String name, MuonZipPath newRoot) {
		super(MuonZipFileSystem.class, MuonZipPath.class, name, true);
	...
	private void addFolder(MuonZipPath src, MuonZipPath dst) {
		MuonZipFileSystem srcFS = src.fs;
		QuiltUnifiedEntry entryFrom = srcFS.getEntry(src);
		if (entryFrom instanceof QuiltUnifiedEntry.QuiltUnifiedFolderReadOnly) {
			// QuiltZipFolder does store subfolders that are part of the original FS, so we need to fully copy it
			MuonMapPath<?, ?>[] srcChildren = ((QuiltUnifiedEntry.QuiltUnifiedFolderReadOnly) entryFrom).children;
			MuonMapPath<?, ?>[] dstChildren = new MuonMapPath<?, ?>[srcChildren.length];
			for (int i = 0; i < srcChildren.length; i++) {
				MuonMapPath<?, ?> srcChild = srcChildren[i];
				MuonZipPath dstChild = dst.resolve(srcChild.name);
				addFolder((MuonZipPath) srcChild, dstChild);
				dstChildren[i] = dstChild;
			}
			addEntryWithoutParentsUnsafe(new QuiltUnifiedEntry.QuiltUnifiedFolderReadOnly(dst, dstChildren));
	 ...
	 * {@link #MuonZipFileSystem(String, Path, String)} - likely more quickly than a regular zip file. The source must
	...
	@Override
	MuonZipPath createPath(@Nullable MuonZipPath parent, String name) {
		return new MuonZipPath(this, parent, name);
	}
	...
	static abstract class ZipSource {
		...
		abstract void open(MuonZipFileSystem fs);

		abstract void close(MuonZipFileSystem fs) throws IOException;
	...
	static final class InMemorySource extends ZipSource {
		...
		@Override
		void open(MuonZipFileSystem fs) {
			// NO-OP
		}

		@Override
		void close(MuonZipFileSystem fs) throws IOException {
			// NO-OP
		}
	...
	static final class SharedByteChannels extends ZipSource {
		final Path zipFrom;
		final Set<WeakReference<MuonZipFileSystem>> fileSystems = new HashSet<>();
		...
		SharedByteChannels(MuonZipFileSystem fs, Path zipFrom) {
			this.zipFrom = zipFrom;
			channels = new ConcurrentHashMap<>();
			open(fs);
			MuonLoaderCleanupTasks.addCleanupTask(this, this::removeDeadThreads);
		}
		...
		@Override
		synchronized void open(MuonZipFileSystem fs) {
			fileSystems.add(fs.thisRef);
		}

		@Override
		synchronized void close(MuonZipFileSystem fs) throws IOException {
			...
			if (fileSystems.isEmpty()) {
				...
				channels.clear();
				MuonLoaderCleanupTasks.removeCleanupTask(this);
			}
		}
		...
		private synchronized void removeDeadThreads() {
			...
			Iterator<WeakReference<MuonZipFileSystem>> iter = fileSystems.iterator();
			while (iter.hasNext()) {
	...
	static final class QuiltZipFile extends QuiltUnifiedEntry.QuiltUnifiedFile {
		final ZipSource source;
		final long offset;
		final int compressedSize, uncompressedSize;
		final boolean isCompressed;

		QuiltZipFile(MuonZipPath path, ZipSource source, ZipEntry entry, CustomZipInputStream zip) throws IOException {
			super(path);
		...
		QuiltZipFile(
				MuonZipPath path, ZipSource source, long offset, int compressedSize, int uncompressedSize,
		...
		protected QuiltUnifiedEntry createCopiedTo(MuonMapPath<?, ?> newPath) {
			return new QuiltZipFile((MuonZipPath) newPath, source, offset, compressedSize, uncompressedSize, isCompressed);
		}
