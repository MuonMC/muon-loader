...
public abstract class QuiltMapFileSystemProvider<FS extends MuonMapFileSystem<FS, P>, P extends MuonMapPath<FS, P>> extends FileSystemProvider {
	...
	public OutputStream newOutputStream(Path pathIn, OpenOption... options) throws IOException {
		...
		QuiltUnifiedEntry.QuiltUnifiedFile targetFile = null;
		...
		if (create) {
			...
			QuiltUnifiedEntry current = path.fs.getEntry(path);
			if (current instanceof QuiltUnifiedEntry.QuiltUnifiedFile) {
				targetFile = (QuiltUnifiedEntry.QuiltUnifiedFile) current;
			} else {
	...
	protected void ensureWriteable(P path) throws IOException {
		...
		if (rootEntry instanceof QuiltUnifiedEntry.QuiltUnifiedFolderWriteable) {
			return;
		} else if (rootEntry instanceof QuiltUnifiedEntry.QuiltUnifiedFolderReadOnly) {
	...
	public SeekableByteChannel newByteChannel(Path pathIn, Set<? extends OpenOption> options, FileAttribute<?>... attrs)
		...
		if (entry instanceof QuiltUnifiedEntry.QuiltUnifiedFile) {
			return ((QuiltUnifiedEntry.QuiltUnifiedFile) entry).createByteChannel(options);
		} else if (entry != null) {
	...
	public void createDirectory(Path dir, FileAttribute<?>... attrs) throws IOException {
		...
		synchronized (path.fs) {
			...
			ensureWriteable(path);
			path.fs.addEntryRequiringParent(new QuiltUnifiedEntry.QuiltUnifiedFolderWriteable(path));
		}
	}
	...
	private void copy0(P src, P dst, boolean isMove, CopyOption[] options) throws IOException {
		...
		if (!(srcEntry instanceof QuiltUnifiedEntry.QuiltUnifiedFile)) {
			throw new IOException("Not a file: " + src);
		}

		QuiltUnifiedEntry.QuiltUnifiedFile srcFile = (QuiltUnifiedEntry.QuiltUnifiedFile) srcEntry;
