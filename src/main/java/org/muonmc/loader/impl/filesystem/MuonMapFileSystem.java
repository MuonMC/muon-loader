...
public abstract class MuonMapFileSystem<FS extends MuonMapFileSystem<FS, P>, P extends MuonMapPath<FS, P>>
	extends MuonBaseFileSystem<FS, P>
	...
	public MuonMapFileSystem(Class<FS> filesystemClass, Class<P> pathClass, String name, boolean uniqueify) {
	...
	public static void dumpEntries(FileSystem fs, String name) {
		if (ENABLE_DEBUG_DUMPING && fs instanceof MuonMapFileSystem) {
			((MuonMapFileSystem<?, ?>) fs).dumpEntries(name);
		}
	}
	...
	public void validate() {
		...
		for (Entry<P, QuiltUnifiedEntry> entry : entries.entrySet()) {
			...
			if (!path.isRoot()) {
				...
				if (parent == null || !(parent instanceof QuiltUnifiedEntry.QuiltUnifiedFolder)) {
					throw new IllegalStateException("Entry " + path + " doesn't have a parent!");
				}
				QuiltUnifiedEntry.QuiltUnifiedFolder pp = (QuiltUnifiedEntry.QuiltUnifiedFolder) parent;
	...
	private <T extends Throwable> void addEntryRequiringParents0(QuiltUnifiedEntry newEntry, Function<String, T> execCtor) throws T {
		...
		synchronized (this) {
			...
			if (entry instanceof QuiltUnifiedEntry.QuiltUnifiedFolderWriteable) {
				addEntryWithoutParents0(newEntry, execCtor);
				((QuiltUnifiedEntry.QuiltUnifiedFolderWriteable) entry).children.add(path);
			} else if (entry == null) {
	...
	private synchronized <T extends Throwable> void addEntryAndParents0(QuiltUnifiedEntry newEntry, Function<String, T> execCtor) throws T {
		...
		while ((parent = parent.getParent()) != null) {
			...
			QuiltUnifiedEntry.QuiltUnifiedFolderWriteable parentFolder;
			if (parentEntry == null) {
				addEntryWithoutParents0(parentFolder = new QuiltUnifiedEntry.QuiltUnifiedFolderWriteable(parent), execCtor);
			} else if (parentEntry instanceof QuiltUnifiedEntry.QuiltUnifiedFolderWriteable) {
				parentFolder = (QuiltUnifiedEntry.QuiltUnifiedFolderWriteable) parentEntry;
			} else {
	...
	protected synchronized boolean removeEntry(P path, boolean throwIfMissing) throws IOException {
		...
		if (current instanceof QuiltUnifiedEntry.QuiltUnifiedFolder) {
			if (!((QuiltUnifiedEntry.QuiltUnifiedFolder) current).getChildren().isEmpty()) {
		...
		entries.remove(path);
		if (parent instanceof QuiltUnifiedEntry.QuiltUnifiedFolderWriteable) {
			((QuiltUnifiedEntry.QuiltUnifiedFolderWriteable) parent).children.remove(path);
		}
		return true;
	}
	...
	public Iterable<FileStore> getFileStores() {
		FileStore store = new FileStore() {
			@Override
			public String type() {
				return MuonMapFileSystem.this.getClass().getName();
			}
			...
			@Override
			public String name() {
				return MuonMapFileSystem.this.name;
			}

			@Override
			public boolean isReadOnly() {
				return MuonMapFileSystem.this.isReadOnly();
			}
